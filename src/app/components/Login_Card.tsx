  ...
  return (
    <main className="auth-page">
      <form className="auth-card card" onSubmit={handleSubmit} noValidate>
        <Image src={props.image} alt={props.alter} height={48} width={48} />

        <h2>{props.role} Login</h2>
        <p>{props.para}</p>

        <div className="field">
          <label htmlFor="login-id">{props.label}</label>
          <input
            id="login-id"
            type={props.type}
        ...
        <div className="auth-links">
          <Link href="/pages/Chose_Login">Back to login options</Link>
          <Link href="/">Home</Link>
        </div>
      </form>
    </main>
