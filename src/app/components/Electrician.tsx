...
const Electrician = () => {
  ...
  useEffect(() => {
    fetch("/api/applicants")
      ...
      .then((data: Applicant[]) => {
        const filtered = data.filter((a) => a.course.toLowerCase().includes("electrician"));
        setStudents(filtered);
      })
      .catch((err) => {
        console.error("Error fetching electrician students:", err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  if (loading) {
    return <div style={{ padding: "2rem", textAlign: "center" }}>Loading Electrician trade data...</div>;
  }
  ...
  return (
    <>
      ...
      <div>
        <h2>Electrician Students</h2>
      </div>

      <div className="space-y-5" style={{ display: "grid", gap: "0.75rem" }}>
        {students.length === 0 ? (
          <p style={{ color: "var(--muted)" }}>No Electrician students registered yet.</p>
        ) : (
