...
const Fitter = () => {
  ...
  useEffect(() => {
    fetch("/api/applicants")
      ...
      .then((data: Applicant[]) => {
        const filtered = data.filter((a) => a.course.toLowerCase().includes("fitter"));
        setStudents(filtered);
      })
      .catch((err) => {
        console.error("Error fetching fitter students:", err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  if (loading) {
    return <div style={{ padding: "2rem", textAlign: "center" }}>Loading Fitter trade data...</div>;
  }
  ...
  return (
    <>
      ...
      <div>
        <h2>Fitter Students</h2>
      </div>

      <div className="space-y-5" style={{ display: "grid", gap: "0.75rem" }}>
        {students.length === 0 ? (
          <p style={{ color: "var(--muted)" }}>No Fitter students registered yet.</p>
        ) : (
